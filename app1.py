...
def show():
    ...
    g="No Remedies"
    a="Cause:<br /> Fungus (Alternaria cucumerina).<br />Symptoms: Irregular brown spots on the leaves—sometimes with yellow edges.<br />Treatment: Fungicides—either commercial or homemade.<br />  You can also try spraying homemade fungicides made of a mixture of soapy water, baking soda, and vinegar.<br /> Some organic gardeners also opt for copper fungicides.<br />  If only a few parts of the plant are affected, cut and remove those parts to prevent the fungus from spreading. <br /> If the plant is severely infected, you may have to remove the whole plant, treat or replace the soil, and start over."
